...
In linear regession, we are looking for a linear function that will predict the dependent feature (target) based on the independent features.  It is based on a line $Y_i$ = $B_iX_i$ + $B_0$ which represents the degree of change in the dependent features for every unit of change in the independent variable  where

                            $B_i$ is the slope,
                            $X_i$ is the independent variable, and
                            $B_0$is the y -intercept (best line)
The accuracy of predicted values based on actual values are the coefficients of $B_0$ and $B_1$ .
